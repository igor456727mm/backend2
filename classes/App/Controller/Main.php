...
//Include 'login.php' subview
    ...
    public function action_landing() {
        ...
        //$this->view->fragments = $this->pixie->orm->get('fragment')->order_by('frgm_id','desc')->limit(300)->find_all();
    }

    public function action_facecom() {
        define('LANDING', '1');
        $this->view->subview = 'facecom';
        if (!$this->logged_in())
            return $this->redirect('/main/login');
        $user = $this->view->user;
        $this->view->fragments = $user->devices->fragments->
                        order_by('frgm_id', 'desc')->limit(10)->find_all();
        $this->view->personas = $user->objects->where('deleted_fl', 0)->find_all();
    }
